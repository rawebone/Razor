...
edit, get, or remove TODO items then that change comes through this file.
...
GET request, the code in the delegate (or Closure) passed to the `get()`
method is invoked. There are method calls for each of the major HTTP Verbs,
and more may be supported in the future.
...
quite ugly code to lazy load it, and secondly because `function () use (...)` is
really terse, ugly and becomes difficult to maintain and read over time.
...
Please base all pull requests against master.
